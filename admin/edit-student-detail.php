...
if (strlen($_SESSION['sturecmsaid']==0)) {
 ...
 $result=$_POST['result'];
 $eid=$_GET['editid'];
$sql="update tblstudent set StudentName=:stuname,StudentEmail=:stuemail,StudentClass=:stuclass,Gender=:gender,DOB=:dob,StuID=:stuid,FatherName=:fname,MotherName=:mname,ContactNumber=:connum,AltenateNumber=:altconnum,Address=:address,Attendance=:attendance,CGPA=:cgpa,SGPA=:sgpa,Result=:result where ID=:eid";
...
<html lang="en">
  <head>
    ...
    <title>ANITS Student Portal/Update Students</title>
    <!-- plugins:css -->
                ...
                <div class="card">
                  <div class="card-body">
                    ...
                    <form class="forms-sample" method="post" enctype="multipart/form-data">
                      <?php
$eid=$_GET['editid'];
$sql="SELECT tblstudent.StudentName,tblstudent.StudentEmail,tblstudent.StudentClass,tblstudent.Gender,tblstudent.DOB,tblstudent.StuID,tblstudent.FatherName,tblstudent.MotherName,tblstudent.ContactNumber,tblstudent.AltenateNumber,tblstudent.Address,tblstudent.Attendance,tblstudent.CGPA,tblstudent.SGPA,tblstudent.Result,tblstudent.UserName,tblstudent.Password,tblstudent.Image,tblstudent.DateofAdmission,tblclass.ClassName,tblclass.Section from tblstudent join tblclass on tblclass.ID=tblstudent.StudentClass where tblstudent.ID=:eid";
$query = $dbh -> prepare($sql);
$query->bindParam(':eid',$eid,PDO::PARAM_STR);
$query->execute();
...
<option value="<?php echo htmlentities($row1->ClassName);?><?php echo htmlentities($row1->Section);?>"><?php echo htmlentities($row1->ClassName);?> <?php echo htmlentities($row1->Section);?></option>
 <?php } ?> 
				...
				<input type="text" name="stuname" value="<?php  echo htmlentities($row->Attendance);?>" class="form-control" required='true'>
                      </div>
			    <div class="form-group">
                        <label for="exampleInputName1">CGPA</label>
                        <input type="text" name="stuname" value="<?php  echo htmlentities($row->CGPA);?>" class="form-control" required='true'>
                      </div>
			    <div class="form-group">
                        <label for="exampleInputName1">SGPA</label>
                        <input type="text" name="stuname" value="<?php  echo htmlentities($row->SGPA);?>" class="form-control" required='true'>
                      </div>
			   <div class="form-group">
                        <label for="exampleInputName1">Result</label>
                        <input type="text" name="stuname" value="<?php  echo htmlentities($row->Result);?>" class="form-control" required='true'>
                      </div>
<h3>Login details</h3>
<div class="form-group">
